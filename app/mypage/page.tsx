    ...
    <div className="bg-gray-100 from-pink-100 via-purple-100 to-indigo-100">
      <main className="mx-auto max-w-7xl px-4 py-8">
        <div className="text-center mb-8">
          <h2
            className={`text-3xl font-bold relative inline-block ${dotGothic16.className}`}
          >
            マイページ
          </h2>
        ...
        {/* ユーザー情報 + ランクカード統合 */}
        <div
          className={`max-w-md mx-auto rounded-xl shadow-md overflow-hidden border ${cardBgColor} mb-8`}
        >
          <div className="p-8 space-y-4">
            ...
            <div className="text-center space-y-4 bg-white/70 p-6 rounded-xl shadow-md">
              ...
              <h4 className={`text-2xl font-bold ${titleColor} mt-2`}>
                {rankName}
              </h4>
              <p className="text-gray-600 text-sm mt-1">
                {colorLevel === 0 &&
                  "初心者ランクです。これからポイントを貯めていきましょう！"}
                {colorLevel === 1 &&
                  "ブルーランクに到達しました。順調にポイントを貯めています！"}
                {colorLevel === 2 &&
                  "イエローランクに到達しました。素晴らしい進歩です！"}
                {colorLevel === 3 &&
                  "グリーンランクに到達しました。上級者の証です！"}
                {colorLevel === 4 &&
                  "パープルランクに到達しました。エキスパートの証です！"}
              </p>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-100">
              <p className="text-sm text-gray-600">
                現在の色:{" "}
                <span className="font-medium">{currentColorName}</span>
              </p>
              <p className="text-sm text-gray-600">
                次の色変更まであと{" "}
                <span className="font-medium">{pointsToNextColor}</span>{" "}
                ポイント
              </p>
